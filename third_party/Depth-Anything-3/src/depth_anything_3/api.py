...
try:
    ...
    _HAS_POSE_ALIGN = True
except ImportError:
    _HAS_POSE_ALIGN = False
    align_poses_umeyama = None
...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def _align_to_input_extrinsics_intrinsics(
        ...
        if extrinsics is None:
            return prediction
        if not _HAS_POSE_ALIGN:
            logger.warning("pose_align module not available, skipping alignment")
            return prediction
